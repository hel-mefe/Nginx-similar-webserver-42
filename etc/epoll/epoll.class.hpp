...
# include "epoll_manager.struct.hpp"


class Epoll : public MultiplexerInterface
{
    private:
        t_http_configs                          *configs;
        t_epoll_manager                         *manager;
    ...
    public:
        Epoll() : configs(nullptr), servers(nullptr)
        {
        ...
        Epoll(t_http_configs *_configs, std::vector<t_server *>* _servers)
        {
            manager = nullptr;
            configs = _configs;
            servers = _servers;
        }
        Epoll(t_http_configs *_configs, std::vector<t_server *>* _servers, HashMap<int, std::string> *_codes, HashMap<std::string, std::string> *_mimes)
        {
            manager = nullptr;
            configs = _configs;
            servers = _servers;
        ...
        void        handle_connection(t_epoll_manager *manager, SOCKET fd);
        void        handle_disconnection(t_epoll_manager *manager, SOCKET fd);
        void        handle_client(t_epoll_manager *manager, SOCKET fd);
