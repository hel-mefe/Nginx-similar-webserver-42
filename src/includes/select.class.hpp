...
# include "selectManager.struct.hpp"

class Select : public MultiplexerInterface
{
    private:
    ...
    public:
        Select() : configs(nullptr), servers(nullptr)
        {
        ...
        Select(t_http_configs *_configs, std::vector<t_server *>* _servers)
        {
            manager = nullptr;
            configs = _configs;
            servers = _servers;
        }
        Select(t_http_configs *_configs, std::vector<t_server *>* _servers, HashMap<int, std::string> *_codes, HashMap<std::string, std::string> *_mimes)
        {
            manager = nullptr;
            configs = _configs;
            servers = _servers;
        ...
        void        handle_connection(t_select_manager *manager, SOCKET fd);
        void        handle_disconnection(t_select_manager *manager, SOCKET fd);
        void        handle_client(t_select_manager *manager, SOCKET fd);
